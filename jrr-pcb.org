...
#+call: kicad-bom(BOM_COLS="Reference,Value,Footprint,${QUANTITY}")
...
| R1,R2                        | 10k                       | Resistor_SMD:R_0805_2012Metric                             |        2 |
| R3,R4                        | 5k1                       | Resistor_SMD:R_0805_2012Metric                             |        2 |
| SW1,SW2                      | SW_PUSH_THT_6mm           | Button_Switch_THT:SW_PUSH_6mm                              |        2 |


#+call: kicad-2d(LAYERS="Edge.Cuts,User.Drawings,F.Silkscreen,F.Fab,F.Paste", IMAGE_FILE="pics/jrr5-fab_f.png")

#+RESULTS:
[[file:pics/jrr5-fab_f.png]]

#+call: kicad-2d(LAYERS="Edge.Cuts,User.Drawings,B.Silkscreen,B.Fab,B.Paste", IMAGE_FILE="pics/jrr5-fab_b.png")

#+RESULTS:
[[file:pics/jrr5-fab_b.png]]
...
#+ATTR_LATEX: :font \tiny
   ...
   | JRR-12 | E            |           | H{1-6}  | Hole diameter 2.5mm          | --> 4mm                      |
   | JRR-13 | 23.3./2026/E |           |         | Mounting holes too small     | M3.5mm                       |
   | JRR-14 | E            | F         | R1,R2   | 100k                         | 10k                          |
   | JRR-15 | E            |           | D1, D2  | Small pads                   | Increase pad size            |
   | JRR-16 | E            |           | D1, D2  | MPN missing                  | Add MNP                      |
   | JRR-17 | E            |           | F1      | 1.1A                         | 0.75A/PTS181213V075          |
   | JRR-18 | E            |           | J24     | Amp-power                    | on PCB edge                  |
   | JRR-19 | E            |           | C??     | 100uF Cap                    | on 5V-RPI/GND                |
   | JRR-20 | E            |           | Rxx     | <1R Resistro                 | on 5V-RPI                    |
   |--------+--------------+-----------+---------+------------------------------+------------------------------|
...
*** ~kicad-2d~: Create 2d image NAME for SCHEMA_ROOT/SCHEMA_NAME for LAYERS

#+name: kicad-2d
#+BEGIN_SRC bash :eval no-export :results output file :var LAYERS="Edge.Cuts,User.Drawings,F.Silkscreen,F.Fab,F.Paste" SCHEMA_ROOT="schema" SCHEMA_NAME="myclock" NAME=""
  echo "Running in $(pwd) host '$(hostname)' on $(date)" 1>&2
  SCHEMA_FILE=$SCHEMA_ROOT/$SCHEMA_NAME/$SCHEMA_NAME.kicad_pcb
  GERBER_FILE=pics/${SCHEMA_NAME}.grb
  [[ -z "$IMAGE_FILE" ]] && IMAGE_FILE=pics/${SCHEMA_NAME}${NAME:+-}${NAME:=}.png
  echo "LAYERS=$LAYERS" 1>&2
  echo "SCHEMA_ROOT=$SCHEMA_ROOT" 1>&2
  echo "SCHEMA_NAME=$SCHEMA_NAME" 1>&2
  echo "SCHEMA_FILE=$SCHEMA_FILE" 1>&2
  echo "IMAGE_FILE=$IMAGE_FILE" 1>&2
...
Modified:2026-08-24.10:47; # time-stamp
